...
import { StepResult } from '../../../src/report/resultLogger';
...
`;
      ...
      expect(Array.isArray(results)).toBe(true);      expect(results.length).toBeGreaterThan(0);
...
`;
      ...
      expect(results).toBeDefined();
      expect(Array.isArray(results)).toBe(true);      expect(results.length).toBeGreaterThanOrEqual(4);      
      // Check that multiple steps were executed successfully
      const successfulSteps = results.filter((r: StepResult) => r.success === true);
...
`;
      ...
      const stepTexts = results.map((r: StepResult) => r.step || '').filter((text: string) => text.length > 0);
      expect(stepTexts.length).toBeGreaterThan(0);
      
      // Check that at least some steps executed successfully
      const successCount = results.filter((r: StepResult) => r.success).length;
