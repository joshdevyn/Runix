...
`;
      ...
      const attemptedSteps = results.filter((r: StepResult) => r.step && r.step.length > 0);
...
`;
      ...
      const sessionSteps = results.filter((r: StepResult) => 
        r.step && (
...
`;
      ...
      const askSteps = results.filter((r: StepResult) => 
        r.step && (
          r.step.includes('ask') || 
...
`;
      ...
      const askSteps = results.filter((r: StepResult) => r.step && r.step.includes('ask'));
...
`;
      ...
      const modeSteps = results.filter((r: StepResult) => 
...
`;
      ...
      const genSteps = results.filter((r: StepResult) => 
        r.step && (
